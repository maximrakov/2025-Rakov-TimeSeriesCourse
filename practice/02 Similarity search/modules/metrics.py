...
def DTW_distance(ts1: np.ndarray, ts2: np.ndarray, r: float = 1) -> float:
    ...
    # Вычисляем абсолютный размер окна warping window
    if r <= 1:
        # r как доля от длины ряда
        window_size = max(0, int(max(n, m) * r))  # min=0 для r=0
    else:
        # r как абсолютное значение
        window_size = int(r)
    ...
    # Заполняем матрицу DTW с ограничением полосы
    for i in range(1, n + 1):
        # Определяем границы окна для j согласно полосе Сако-Чиба
        ...
        for j in range(j_start, j_end):
            ...
            # Находим минимальный путь из возможных предшественников
